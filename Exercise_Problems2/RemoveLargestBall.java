...
public class RemoveLargestBall extends Application {
  ...
  private class MultipleBallPane extends Pane {
    ...
    public void add() {
      ...
      getChildren().add(new Ball(30, 30, (int)(Math.random() * 20) + 2, color));
    }

    public void subtract() {
      if (getChildren().size() > 0) {
		  double max = 0;
		  Ball[] removeBall = new Ball[1];
          for (int i = 0; i < this.getChildren().size(); i++) {
			  Ball ball = (Ball)this.getChildren().get(i);
			  if (ball.getRadius() > max) {
				  max = ball.getRadius();
				  removeBall[0] = ball;
			  }
		  }
		  this.getChildren().remove(removeBall[0]);
      }
    }
